...
def ml_faixa(item_id):
    ...
    # Buscar dados do item (categoria + seller + dimensões + logística)
    r_item = requests.get(f"https://api.mercadolibre.com/items/{item_id}",
                          headers={"Authorization": f"Bearer {token}"})
    item_data = r_item.json()
    category_id = item_data.get("category_id", "")
    ...
    # Extrair dimensões reais do anúncio (formato: LxAxC,peso_g)
    dimensions = None
    dims_obj = shipping.get("dimensions")
    if dims_obj:
        dimensions = dims_obj  # já vem no formato "30x20x10,500"
    ...
    # Passa dimensões + logística reais para bater com o simulador web do ML
